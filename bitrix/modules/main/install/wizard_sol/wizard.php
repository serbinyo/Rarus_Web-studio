...
class CSelectSiteWizardStep extends CWizardStep
{
    ...
    function OnPostForm()
    {
        ...
        if ($wizard->IsNextButtonClick()) {
            ...
            $siteFolder = str_replace(["\\", "///", "//"], "/", "/" . $wizard->GetVar("siteFolder") . "/");
            ...
            if ($createSite == "Y") {
                ...
                $db_res = CSite::GetList($by = "sort", $order = "desc", ["LID" => $siteID]);
    ...
    function ShowStep()
    {
        ...
        $arSites = [];
        $arSitesSelect = [];
        $db_res = CSite::GetList($by = "sort", $order = "desc", ["ACTIVE" => "Y"]);
...
function SelectCreateSite(element, solutionId)
{
	...
	if (check)
        ...
        $this->content .= $this->ShowRadioField("createSite", "N",
            (["id" => "createSiteN", "class" => "solution-radio"] +
                ($createSite != "Y" ? ["checked" => "checked"] : [])));
        $this->content .= '<h4>' . GetMessage("wiz_site_existing") . '</h4>';
        if (count($arSites) < 2) {
            $this->content .= '<p>' . GetMessage("wiz_site_existing_title") . ' ' . implode("",
                    $arSitesSelect) . '</p>';
        } else {
        ...
        $this->content .= '</div>';
        if (count($arSites) < COption::GetOptionInt("main", "PARAM_MAX_SITES", 100) || COption::GetOptionInt("main",
                "PARAM_MAX_SITES", 100) <= 0) {
            $this->content .= "<div onclick=\"SelectCreateSite(this, 'Y');\" ";
            $this->content .= 'class="solution-item' . ($createSite == "Y" ? " solution-item-selected" : "") . '">';
            $this->content .= '<b class="r3"></b><b class="r1"></b><b class="r1"></b>';
            $this->content .= '<div class="solution-inner-item">';
            $this->content .= $this->ShowRadioField("createSite", "Y",
                (["id" => "createSiteY", "class" => "solution-radio"] +
                    ($createSite == "Y" ? ["checked" => "checked"] : [])));
            ...
            $this->content .= str_replace(
                ...
                [
                    $this->ShowInputField("text", "siteNewID", ["size" => 2, "maxlength" => 2, "id" => "siteNewID"]),
                    $this->ShowInputField("text", "siteFolder", ["id" => "siteFolder"]),
                ],
...
class CSelectTemplateWizardStep extends CWizardStep
{
    ...
    function ShowStep()
    {
        ...
        foreach ($arTemplates as $templateID => $arTemplate) {
            ...
            $this->content .= $this->ShowRadioField("templateID", $templateID,
                ["id" => $templateID, "class" => "inst-template-list-inp"]);
            if ($arTemplate["SCREENSHOT"] && $arTemplate["PREVIEW"]) {
                $this->content .= CFile::Show2Images($arTemplate["PREVIEW"], $arTemplate["SCREENSHOT"], 150, 150,
                    ' class="inst-template-list-img"');
            } else {
                $this->content .= CFile::ShowImage($arTemplate["SCREENSHOT"], 150, 150,
                    ' class="inst-template-list-img"', "", true);
            }
        ...
        foreach ($arThemes as $themeID => $arTheme) {
            ...
            $this->content .= '
					<span class="inst-template-color' . ($defaultThemeID == $themeID ? " inst-template-color-selected" : "") . '" ondblclick="SubmitForm(\'next\');"  onclick="SelectTheme(this, \'' . $themeID . '\', \'' . $arTheme["SCREENSHOT"] . '\');">
						<span class="inst-templ-color-img">' . CFile::ShowImage($arTheme["PREVIEW"], 70, 70,
                    ' border="0" class="solution-image"') . '</span>
						<span class="inst-templ-color-name">' . $ii . '. ' . $arTheme["NAME"] . '</span>
					</span>';
        }

        $this->content .= $this->ShowHiddenField($themeVarName, $defaultThemeID, ["id" => "selected-solution"]);
        $this->content .=
            '</div>' .
...
class CDataInstallWizardStep extends CWizardStep
{
    ...
    function OnPostForm()
    {
        ...
        if ($site_id != "") {
            $db_res = CSite::GetList($by = "sort", $order = "desc", ["LID" => $site_id]);
            if ($db_res) {
                $res = $db_res->Fetch();
            }
        }
        ...
        if (!$this->repeatCurrentService) {
            list($nextService, $nextServiceStage, $stepsComplete, $status) =
                $this->GetNextStep($arServices, $serviceID,
                $serviceStage);
        }

        if ($nextService == "finish") {
            $response = "window.ajaxForm.StopAjax(); window.ajaxForm.SetStatus('100'); window.ajaxForm.Post('"
                . $nextService . "', '" . $nextServiceStage . "','" . $status . "');";
            COption::SetOptionString("main",
                "wizard_first" . substr($wizard->GetID(), 7) . "_" . $wizard->GetVar("siteID"), "Y", false);
        } else {
            ...
            $response = ($percent ? "window.ajaxForm.SetStatus('" . $percent . "');" : "")
                . " window.ajaxForm.Post('" . $nextService . "', '" . $nextServiceStage . "','" . $status . "');";
        }
    ...
    function InstallService($serviceID, $serviceStage)
    {
        ...
        define("WIZARD_SITE_ROOT_PATH", $WIZARD_SITE_ROOT_PATH);
        define("WIZARD_SITE_PATH", str_replace("//", "/"
            , WIZARD_SITE_ROOT_PATH . "/" . WIZARD_SITE_DIR . "/"));
        ...
        define("WIZARD_FIRST_INSTAL", COption::GetOptionString("main", "wizard_first"
            . substr($wizard->GetID(), 7) . "_" . $wizard->GetVar("siteID")
            , false, $wizard->GetVar("siteID")));

        $dbUsers = CGroup::GetList($by = "id", $order = "asc", ["ACTIVE" => "Y"]);
    ...
    function GetNextStep(&$arServices, &$currentService, &$currentStage)
    {
        ...
        if (!array_key_exists($currentService, $arServices)) {
            return [$nextService, $nextServiceStage, 0, $status];
        } //Finish

        if ($currentStage != "skip" && array_key_exists("STAGES",
                $arServices[$currentService]) && is_array($arServices[$currentService]["STAGES"])) {
        ...
        if (array_key_exists("STAGES",
                $arServices[$nextServiceID]) && is_array($arServices[$nextServiceID]["STAGES"])
            && isset($arServices[$nextServiceID]["STAGES"][0])) {
        ...
        return [
            $nextServiceID,
            $nextServiceStage,
            $arServices[$nextServiceID]["POSITION"] - 1,
            $arServices[$nextServiceID]["NAME"],
        ]; //Next service
    }
    ...
    function GetFirstStep(&$arServices)
    {
        foreach ($arServices as $serviceID => $arService) {
            $stage = "index.php";
            if (array_key_exists("STAGES",
                    $arService) && is_array($arService["STAGES"]) && isset($arService["STAGES"][0])) {
                $stage = $arService["STAGES"][0];
            }
            return [$serviceID, $stage, $arService["NAME"]];
        }

        return ["service_not_found", "finish", GetMessage("INSTALL_SERVICE_FINISH_STATUS")];
    }
}
...
class CFinishWizardStep extends CWizardStep
{
    ...
    function CreateNewIndex()
    {
        ...
        define("WIZARD_SITE_ROOT_PATH", $WIZARD_SITE_ROOT_PATH);
        define("WIZARD_SITE_PATH", str_replace("//", "/", WIZARD_SITE_ROOT_PATH
            . "/" . WIZARD_SITE_DIR . "/"));
